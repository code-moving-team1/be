...
// 1. 리프레쉬 토큰 생성
export async function createRefreshToken(userId: number, userType: UserType, expiresAt: Date, refreshToken: string) {
...
// 2. 리프레쉬 토큰 찾기
export async function findByUserIdAndUserType(userId: number, userType: UserType) {
...
export async function isSameRawToken(hashed: string, raw: string) {
  ...
  // Sliding
export async function saveSlidingRT(userId: number, userType: UserType, opaqueId: string, refreshToken: string, userAgent: string, ip: string, expiresAt: Date) {
  const hashed = await bcrypt.hash(refreshToken, 12) as string
  return prisma.refreshToken.create({
    data: { userId, userType, opaqueId, hashed, userAgent, ip, expiresAt, lastUsedAt: new Date() },
  });
}
...
export async function updateSlidingOnUse(id: number, { extendsMs }: { extendsMs: number }) {
  const now = Date.now();
